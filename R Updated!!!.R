...
###Read in all the data sets from excel###
##########################################
...
##Join the data sets by the three columns####################
...
##Change the formating of doy##
##List by year, month, day#####
###############################
...
##In new data sets rename colums by NDVI and doy##
##Repeat for four data sets with two columns per##
...
##Aggregate into one larger data set##
######################################
...
##Why are green and red higher than blue and black#################
##At low density stand the NDVI is higher than high density stand##
##Less plants but healthier at low density#########################
##Is sattelite data typically lower in low density than high?######
...
##Leaf Wetness Sensor###
########################
datAll.LWS <- ifelse(datAll$LWS.minDU<0|datAll$LWS.minDU>250, NA, datAll$LWS.LWDU)
...
plot(datJoin$doy, datJoin$NDVI.dc, type="l",
     col="black",xlab = "Day of Year", ylab = "NDVI", 
        ylim = c(0,1))
points(AveLWSDU$doy, AveLWSDU$LWS, type="l", col="blue")
